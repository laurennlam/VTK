  ...
  const double inX, const double inY, double& outX, double& outY)
{
  if (this->HandleOrientation == vtkPlotRangeHandlesItem::VERTICAL)
  {
    Superclass::TransformScreenToData(inX, inY, outX, outY);
  }
  else
  {
    Superclass::TransformScreenToData(inY, inX, outY, outX);
  }
}
  ...
  const double inX, const double inY, double& outX, double& outY)
{
  if (this->HandleOrientation == vtkPlotRangeHandlesItem::VERTICAL)
  {
    Superclass::TransformDataToScreen(inX, inY, outX, outY);
  }
  else
  {
    Superclass::TransformDataToScreen(inY, inX, outY, outX);
  }
}
...
void vtkPlotRangeHandlesItem::GetHandlesRange(double range[2])
{
  if (this->ActiveHandle != vtkPlotRangeHandlesItem::NO_HANDLE)
  {
    ...
    if (this->SynchronizeRangeHandles &&
      this->ActiveHandle == vtkPlotRangeHandlesItem::LEFT_HANDLE)
    {
